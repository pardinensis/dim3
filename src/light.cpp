...
Light::Light(Type type, const glm::vec3& pos, const glm::vec3& dir, float radius, const glm::vec4& color,
...
Light* create_spotlight(const glm::vec3& pos, const glm::vec3& dir, float radius, const glm::vec4& color,
		float inner_angle, float outer_angle) {
...
void bind_lights_to_shader(GLuint shader_id) {
	...
	if (n_pl) {
		float pl_pos[3 * n_pl], pl_radius[n_pl], pl_col[4 * n_pl];
		for (int i = 0; i < n_pl; ++i) {
			...
			pl_col[4 * i + 0] = l->color.x;
			pl_col[4 * i + 1] = l->color.y;
			pl_col[4 * i + 2] = l->color.z;
			pl_col[4 * i + 3] = l->color.w;
		}
		glUniform3fv(shader::uniform(shader_id, "pl_pos"), n_pl, pl_pos);
		glUniform1fv(shader::uniform(shader_id, "pl_radius"), n_pl, pl_radius);
		glUniform4fv(shader::uniform(shader_id, "pl_col"), n_pl, pl_col);
