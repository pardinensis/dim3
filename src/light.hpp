...
protected:
	...
	glm::vec3 color;	// POINT  SPOT  GLOBAL
	float inner_angle;	//        SPOT
	float outer_angle;	//        SPOT

protected:
	Light(Type type, const glm::vec3& pos, const glm::vec3& dir, float radius, const glm::vec3& color,
		float inner_angle, float outer_angle);

	friend Light* create_pointlight(const glm::vec3& pos, float radius, const glm::vec3& color);
	friend Light* create_spotlight(const glm::vec3& pos, const glm::vec3& dir, float radius,
		const glm::vec3& color, float inner_angle, float outer_angle);
	friend Light* create_globallight(const glm::vec3& dir, const glm::vec3& color);
